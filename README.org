...
screen_file="$screen_dir/screenshot-$(~/.local/bin/get-timestamp.sh).$screen_format"
...
case $capture_target in
    ...
    screen)
        # active_monitor=$(hyprctl -j activewindow | jq -r .monitor)
        # grim -g "$(hyprctl -j monitors | jq -r '.[] | select(.id | contains(0))' | jq -r '"\(.x),\(.y) \(.width)x\(.height)"')" "$screen_file" || exit 1
        grim "$screen_file" || exit 1
        message="Main monitor"
        ;;
    window)
        message="Active window"
        grim -g "$(hyprctl -j activewindow | jq -r '"\(.at[0]),\(.at[1]) \(.size[0])x\(.size[1])"')" "$screen_file" || exit 1
        ;;
    area)
        message="Area selection"
        grim -g "$(slurp)" "$screen_file" || exit 1
        ;;
    ,*)
        echo -e """Only the following arguments are accepted:
    full
...
paplay "$shutter"
notify-send --urgency-low "Screenshot saved!" "$message"
#+end_src
