...
* dm-theme
Changes the color scheme of all applications, configured in the [[#setup-7][Setup]] section,
for a more unified look of your desktop environment. Each application needs to
have themes configured/available for it to be affected, so a bit of manual work
is still required in setting up available themes. Once that is done though,
...
readme["qutebrowser"]="$config_path/README.org"
config["qutebrowser"]="$config_path/config.py"
t_path["qutebrowser"]="$config_path/colors"
regexp["qutebrowser"]="\(import colors\.\)\w\+\( as theme\)"
#+end_src
...
names.
=awk= expression removed file extensions so avoid having extra dots in the file
